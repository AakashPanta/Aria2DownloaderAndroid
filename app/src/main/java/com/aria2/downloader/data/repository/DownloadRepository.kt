...
class DownloadRepository @Inject constructor(
    ...
    fun observeRunning(): Flow<List<DownloadInfo>> = downloadDao.observeRunning().map { list -> list.map { it.toDomain() } }

    fun observeQueued(): Flow<List<DownloadInfo>> = downloadDao.observeQueued().map { list -> list.map { it.toDomain() } }
